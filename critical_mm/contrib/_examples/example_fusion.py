...
``augment`` receives the preprocessed per-split feature frames plus the optional
ICD and notes blocks (polars frames keyed on the hashed ``GROUP`` id; per-hour
blocks also carry the ``SEQUENCE`` column) and returns augmented frames. The
...
from critical_mm.fusion.strategy import FusionStrategy
