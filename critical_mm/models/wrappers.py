...
from critical_mm.models.constants import DLMetrics, ImputationInit, MLMetrics
from critical_mm.models.utils import create_optimizer, create_scheduler
...
class DLWrapper(BaseModule, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if loss is None:
            loss = F.cross_entropy if run_mode == RunMode.classification else F.mse_loss
        self.loss = loss
    ...
    def finalize_step(self, step_prefix=""):
        ...
        for name, metric in self.metrics[step_prefix].items():
            if "_Curve" in name:
                continue
            try:
                computed[f"{step_prefix}/{name}"] = metric.compute()
                ...
                if step_prefix not in self._metrics_warning_printed:
                    ...
                    logging.warning(f"Some metrics for {step_prefix} not computable")
                continue
        if computed:
    ...
    def configure_optimizers(self):
        if isinstance(self.optimizer, str):
            optimizer = create_optimizer(
                self.optimizer, self, self.hparams.lr, self.hparams.momentum
            )
        else:
    ...
    def save_model(self, save_path, file_name, file_extension=".ckpt"):
        ...
        try:
            ...
            logging.info(f"Model saved to {path.resolve()!s}.")
        except Exception as e:
            logging.error(f"Cannot save model to path {path.resolve()!s}: {e}.")
...
class DLPredictionWrapper(DLWrapper):
    ...
    def step_fn(self, element, step_prefix=""):
        ...
        if len(out) == 2 and isinstance(out, tuple):
            ...
            aux_loss = 0
        prediction = (
            torch.masked_select(out, mask.unsqueeze(-1)).reshape(-1, out.shape[-1]).to(self.device)
        )
        target = torch.masked_select(labels, mask).to(self.device)
        if prediction.shape[-1] > 1 and self.run_mode == RunMode.classification:
            loss = (
                self.loss(prediction, target.long(), weight=self.loss_weights.to(self.device))
                + aux_loss
            )
        elif self.run_mode == RunMode.regression:
...
class MLWrapper(BaseModule, ABC):
    ...
    def __init__(
        self, *args, run_mode=RunMode.classification, loss=None, patience=10, **kwargs
    ):
        super().__init__()
        ...
        self.run_mode = run_mode
        if loss is None:
            loss = log_loss if run_mode == RunMode.classification else mean_squared_error
        self.loss = loss
        ...
        self.log_dict(
            {
                ...
                if not isinstance(
                    metric(self.label_transform(label), self.output_transform(pred)), tuple
                )
            },
            sync_dist=True,
        )
