...
def parallel_recomb(q_t, kv_t, att_type="all", local_context=3, bin_size=None):
    ...
    with torch.no_grad():
        q_t[q_t == -1.0] = float("inf")
        kv_t[kv_t == -1.0] = float("inf")

        if bin_size is not None:
            q_t = q_t / bin_size
            starts_q = q_t[:, 0:1].clone()
            q_t -= starts_q
            kv_t = kv_t / bin_size
            starts_kv = kv_t[:, 0:1].clone()
            kv_t -= starts_kv
        ...
        if att_type == "local":
            return (
                (diff_mask >= 0) * (diff_mask <= local_context) + (diff_mask == float("inf"))
            ).float()
        if att_type == "strided":
            return (
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mask:
            if isinstance(self.att_type, list):
                if self.mask_aggregation == "union":
                    mask_tensor = 0
                    for att_type in self.att_type:
                        mask_tensor += parallel_recomb(
                            torch.arange(1, n + 1, dtype=torch.float, device=dot.device).reshape(
                                1, -1
                            ),
                            torch.arange(1, n + 1, dtype=torch.float, device=dot.device).reshape(
                                1, -1
...
class SparseBlock(nn.Module):
    def __init__(
        ...
        self.ff = nn.Sequential(
            nn.Linear(emb, ff_hidden_mult * emb), nn.ReLU(), nn.Linear(ff_hidden_mult * emb, emb)
        )
