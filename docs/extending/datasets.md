...
Different datasets have different table coverage. The capability mechanism formalizes this with a `CAPABILITIES` ClassVar:
...
Cohorts and splits are read from two independent roots, and mixing them is the most common way to
get a silently wrong run.

- **Cohort root** — where `sta.parquet` / `dyn.parquet` / `outc.parquet` live for a
  `(task, dataset)` pair. Resolved by `critical_mm/io/paths.py` under `CRITICAL_MM_DATA_ROOT`.
- **Split root** — where the locked fold assignments live. Passed explicitly via
  `--splits-root`, never inferred. The patient-grouped tree is
  `data/processed/splits_patient/`.

**Read cohorts for every dataset from the same root.** A cohort tree built separately from the one
the splits were locked against will have a disjoint `stay_id` namespace. The failure is quiet: the
join drops every row, and you get an empty training set or — worse — statics that silently belong
to different stays. `critical_mm/fusion/blocks.py` raises on the empty-intersection case rather
than training on nothing, which is the error that sent you here.

If you hit it, check in this order:

1. Is `--splits-root` the tree you actually locked, and is it **absolute**?
2. Were the cohorts rebuilt after the splits were locked? If so, re-lock before retraining —
   otherwise cells copy a stale hash from the manifest and reuse checkpoints trained on the old
   cohort.
3. Do the manifest hashes still match the cohort on disk? Check with
   `critical_mm.validation.cohort_fingerprint.checkpoint_is_current`, which compares cohort hashes,
   split hashes and a builder-code hash. A missing hash counts as stale, not as a pass.
